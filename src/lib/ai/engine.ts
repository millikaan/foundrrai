...
DESIGN BAR — this is the whole point, make it world-class:
- VISUAL SYSTEM: commit to ONE cohesive art direction that fits the business (e.g. warm-editorial, clean-modern, bold-premium, soft-minimal). ONE confident accent colour + a neutral scale, defined via CSS variables and the Tailwind theme. A consistent 4/8px spacing rhythm, a clear type scale, and a unified corner-radius + shadow system. Never default-blue, never muddy, never a generic-AI-template look.
- TYPOGRAPHY: load a high-quality Google Font (Inter, Sora, Manrope, Plus Jakarta Sans, or Space Grotesk for display) via <link> in index.html and set it in the Tailwind theme. Big confident headings with clamp(), tight tracking on large display text, comfortable 1.5–1.7 body line-height, a real visual hierarchy (eyebrow / headline / subhead / body).
- DEPTH & POLISH: layered soft shadows, tasteful gradients/blur accents, hairline borders, and a glassy sticky header (backdrop-blur). Generous whitespace, strong hierarchy, and asymmetry where it elevates. NEVER a cramped, plain, centered single column; NEVER three identical filler cards.
- REAL IMAGERY (critical — empty pages look cheap): high-quality, on-topic photographs from Unsplash via real working URLs like https://images.unsplash.com/photo-XXXX?auto=format&fit=crop&w=1600&q=80 with object-cover and correct aspect ratios — a striking hero visual, section imagery, and a gallery where it fits. Tasteful gradients/SVG accents in addition, not instead.
- MOTION (CSS only — add NO animation libraries): smooth hover lifts on cards/buttons, clear focus rings, and a subtle on-scroll reveal (small fade + translate via a tiny IntersectionObserver or CSS). Keep it refined and ALWAYS respect prefers-reduced-motion.
- SECTIONS (vary the rhythm, don't stack identical blocks): sticky blur header + nav, a strong hero (sharp headline + subhead + ONE primary CTA + supporting visual), a benefits/value section, real services or pricing with AZN prices, social proof / testimonials with names + photos, a gallery, an FAQ, and a rich multi-column footer with contact + map link + socials.
...
CONVERSION & COPY — write like a senior Azerbaijani copywriter, not a generator: a sharp, benefit-led hero headline (never bland like "Bizim sayt"), a one-line value proposition, concrete trust signals (illər təcrübə, müştəri sayı, zəmanət, iş saatları), and ONE clear primary action repeated sensibly down the page. Use real specifics for the business type — never vague filler.
...
QUALITY: preserve and elevate the existing design system — never downgrade to a plain/unstyled look. Match the existing palette, fonts, spacing rhythm, radius and component style EXACTLY so anything new looks native to the site, not bolted on. Keep Tailwind wired correctly and the project building with ZERO Vite/PostCSS/TS errors (inside '@apply' use only real utilities — no slash-opacity like 'bg-white/72', no arbitrary values; use rgba()/hsl() alpha for translucency). Use realistic Azerbaijani content (Baku, ₼ prices, +994 numbers) and real Unsplash images (https://images.unsplash.com/photo-...?auto=format&fit=crop&w=1600&q=80) for any new imagery. Every visible word stays natural Azerbaijani.`;

/**
 * Shared branded email layout — matches the Foundrr landing aesthetic (warm-white
 * paper, the vivid blue→violet→pink→orange gradient, dark headline, dark CTA).
 * Email-safe: a single full HTML document, table layout, inline styles only, with
 * solid-color fallbacks for clients (Outlook) that don't render CSS gradients.
 */
  ...
  const cta =
    c.ctaLabel && c.ctaUrl
      ? `<table role="presentation" cellpadding="0" cellspacing="0" style="margin:26px 0 2px;"><tr>
            <td style="border-radius:14px;background-color:#16131f;">
...
<body style="margin:0;padding:0;background:#faf8f5;-webkit-font-smoothing:antialiased;">
  <span style="display:none!important;opacity:0;color:#faf8f5;font-size:1px;line-height:1px;max-height:0;max-width:0;overflow:hidden;">${esc(
    c.preheader,
  )}</span>
    ...
    <tr><td align="center">
      <table role="presentation" width="520" cellpadding="0" cellspacing="0" style="max-width:520px;width:100%;background:#ffffff;border:1px solid #efe9e1;border-radius:24px;overflow:hidden;box-shadow:0 24px 60px -34px rgba(40,28,20,0.20);">
        ...
        <tr><td style="padding:24px 38px 0;font-family:${FONT};">
          <h1 style="margin:0;font-size:24px;line-height:1.25;font-weight:700;letter-spacing:-0.015em;color:#1b1830;">${esc(
            c.heading,
          )}</h1>
          <p style="margin:14px 0 0;font-size:15.5px;line-height:1.65;color:#55506a;">${esc(c.intro)}</p>
          ${c.bodyHtml ?? ""}
          ${cta}
          ${
            c.footnote
              ? `<p style="margin:18px 0 0;font-size:13px;line-height:1.6;color:#8b859c;">${esc(c.footnote)}</p>`
              : ""
          }
        </td></tr>
        <tr><td style="padding:30px 38px 34px;font-family:${FONT};">
          <div style="height:1px;background:#efe9e1;"></div>
          <p style="margin:18px 0 0;font-size:12px;line-height:1.6;color:#a39db4;">
            Foundrr · Bakı, Azərbaycan<br/>
            <a href="${SITE_URL}" style="color:#7735e9;text-decoration:none;">foundrr.online</a> — fikrini yaz, saytın hazır olsun.
          </p>
        </td></tr>
      </table>
      <p style="margin:18px 0 0;font-family:${FONT};font-size:11px;color:#b8b2c2;">© Foundrr</p>
    </td></tr>
  </table>
</body></html>`;
}
